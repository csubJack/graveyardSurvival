...
void initSlimes() 
{
    if (gl.current_level == 1 || gl.current_level == 3) {
        for (int i = 0; i < 1; i++) { // changed to zerof to remove slimes for movement testing
            ...
            s->pos[0] = g.player.pos[0] + cos(ang) * dist;
            s->pos[1] = g.player.pos[1] + sin(ang) * dist;
            s->pos[2] = 0.0f;
...
void spawnZombie() {
    for (int i = 0; i < MAX_ZOMBIES; i++) {
        if (!zombies[i].active) {
            ...
            Vec dir;
            VecSub(g.player.pos, zombies[i].pos, dir);
            normalize2d(dir);
...
// resseting variables
void reset_game_animation() {
    g.player.pos[0] = gl.xres / 2;
    g.player.pos[1] = gl.yres / 2;
    g.player.vel[0] = 0.0f;
    g.player.vel[1] = 0.0f;
...
void check_mouse(XEvent *e)
{
	...
	if (e->type == MotionNotify) {
		// g.player.angle = update_player_angle(e->xmotion.x,
        //         g.player.pos[0], e->xmotion.y, g.player.pos[1]);
	}
	...
	if (savex != e->xbutton.x || savey != e->xbutton.y) {
		...
		if (ydiff > 0) {
			...
			Flt xdir = cos(rad);
			Flt ydir = sin(rad);
			g.player.pos[0] += xdir * 2.0f;
			g.player.pos[1] += ydir * 2.0f;
...
void physics_hat()
{
    ...
    if (gl.current_level == 2 && (!gl.game_paused)) {
        ...
        hat.pos[1] < -100 || hat.pos[1] > gl.yres + 100) {
            ...
            float dx = g.player.pos[0] - hat.pos[0];
            float dy = g.player.pos[1] - hat.pos[1];
    ...
    float dist = sqrt (
            (hat.pos[0] - g.player.pos[0]) * (hat.pos[0] - g.player.pos[0])
            + (hat.pos[1] - g.player.pos[1]) * (hat.pos[1] - g.player.pos[1])
            );
...
void witch_phys()
{
    ...
    if (gl.current_level == 4 && (!gl.game_paused)) {
        ...
        wich.pos[1] < -100 || wich.pos[1] > gl.yres + 100) {
            ...
            // Calculate direction to player
            float dx = g.player.pos[0] - wich.pos[0];
            float dy = g.player.pos[1] - wich.pos[1];
            float dist = sqrt(dx * dx + dy * dy);
            if (dist != 0) {
                dx /= dist;
                dy /= dist;
    ...
    float dist = sqrt (
            (wich.pos[0] - g.player.pos[0]) * (wich.pos[0] - g.player.pos[0])
            + (wich.pos[1] - g.player.pos[1]) * (wich.pos[1] - g.player.pos[1])
            );
...
if (gl.current_level != previous_level) {
    ...
    if (g.player.pos[0] < 0.0) {
        g.player.pos[0] = 0.0; // Stop at the left edge
        g.player.vel[0] = 0.0; // Set velocity to zero
    }
    else if (g.player.pos[0] > (float)gl.xres) {
        g.player.pos[0] = (float)gl.xres; // Stop at the right edge
        g.player.vel[0] = 0.0; // Set velocity to zero
    }

    if (g.player.pos[1] < 0.0) {
        g.player.pos[1] = 0.0; // Stop at the bottom edge
        g.player.vel[1] = 0.0; // Set velocity to zero
    }
    else if (g.player.pos[1] > (float)gl.yres) {
        g.player.pos[1] = (float)gl.yres; // Stop at the top edge
        g.player.vel[1] = 0.0; // Set velocity to zero
    }
    ...
    //current g.player.angle, changing that gl.witch.angle and so on
    ...
    if (gl.keys[XK_s] && gl.game_paused) {
		...
		Flt xdir = -cos(rad);
		Flt ydir = -sin(rad);
		// J player movemnt, Swtiched player.vel to player.pos to remove drift effect
		g.player.pos[0] += xdir * 2.0f;
		g.player.pos[1] += ydir * 2.0f;
	}
    ...
    while (s) {
        ...
        if (s != NULL) {
            Flt d0 = g.player.pos[0] - s->pos[0];
            Flt d1 = g.player.pos[1] - s->pos[1];
            ...
            if (dist < (s->radius + gl.player_size)) {  // 10 is approximate player radius
                ...
                // if (dist > 0) {
                //     g.player.vel[0] += (d0/dist) * knockback;
                //     g.player.vel[1] += (d1/dist) * knockback;
                // }
...
for (int i = 0; i < MAX_ZOMBIES; i++) {
    if (zombies[i].active) {
        float dx = g.player.pos[0] - zombies[i].pos[0];
        float dy = g.player.pos[1] - zombies[i].pos[1];
        float dist = sqrt(dx*dx + dy*dy);
        float zombie_radius = 20.0f;
...
if (gl.game_started) {
    ...
    if (gl.game_paused == true) {
        ...
        draw_player_health_bar(g.player.pos[0], g.player.pos[1], gl.player_health, 10);
